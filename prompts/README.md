...
| [`04-cost-shield.md`](./04-cost-shield.md) | Three moves from one Phase 10/11 conversation: (1) a diagnostic question ("why don't tests trigger LLM?") that surfaced a doc claim — Layer 2's "5 KB residual gap" trigger — that had never been implemented; (2) the cap bump from 1 → 3 calls/request, promoting a magic number to `MAX_LLM_CALLS_PER_REQUEST` and letting both LLM layers fire in the same request; (3) "做快一點，不要浪費我時間" — the user's pick of A+B+F (per-IP rate limit + daily LLM budget + `client_ip` in logs) from a six-option menu, with C/D/E explicitly de-scoped. The thread's pattern: diagnostic questions surface invisible drift; de-scoping is a feature; in-process state should be honestly documented, not dressed up as a security boundary. |
